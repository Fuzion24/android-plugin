...
object AAPT {
 ...
 Modifiers:
   ...
   */
  def apply(manifestPackage:String,
            aaptPath:File,
            manifestPath:Seq[File],
            resPath:Seq[File],
            libraryJarPath:File,
            managedJavaPath:File,
            generatedProguardConfigPath:File,
            aarlibDependencies:Seq[AndroidPlugin.LibraryProject],
            apklibDependencies:Seq[AndroidPlugin.LibraryProject],
            apklibSourceManaged:File,
            useDebug:Boolean):Seq[File] = {

    // Create the managed Java path if necessary
    managedJavaPath.mkdirs
    ...
    val extlibs = apklibDependencies ++ aarlibDependencies
    ...
    runAapt(manifestPackage, managedJavaPath)

    // Run aapt to generate resources for each apklib dependency
    apklibDependencies.foreach(lib => runAapt(lib.pkgName, apklibSourceManaged, "--non-constant-id"))

    def createBuildConfig(`package`: String) = {
      var path = managedJavaPath
    ...
    (managedJavaPath ** "R.java" get) ++
      (apklibSourceManaged ** "R.java" get) ++
      Seq(createBuildConfig(manifestPackage)) ++
      apklibDependencies.map(lib => createBuildConfig(lib.pkgName))
  }
